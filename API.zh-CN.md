...
轻量级游标，用于在树遍历中追踪位置。始终通过 `MA_STATE()` 宏在栈上分配。

| 字段     | 类型                 | 说明                                    |
|----------|----------------------|-----------------------------------------|
| `tree`   | `struct maple_tree*` | 该游标操作的树。                         |
| `index`  | `uint64_t`           | 搜索/存储区间的起始索引。                |
| `last`   | `uint64_t`           | 搜索/存储区间的结束索引（包含）。         |
| `node`   | `struct maple_node*` | 当前节点（首次 walk 前为 NULL）。         |
| `min`    | `uint64_t`           | 通过 `node` 可到达的最小索引。            |
| `max`    | `uint64_t`           | 通过 `node` 可到达的最大索引。            |
| `offset` | `uint8_t`            | 在 `node` 中的槽位偏移。                 |
| `depth`  | `uint8_t`            | 当前深度（0 = 根节点）。                  |
...
| 名称               | 值                | 说明                          |
|--------------------|-------------------|-------------------------------|
| `MAPLE_NODE_SLOTS` | 16                | 分支因子（每节点的槽位数）。    |
| `MAPLE_NODE_PIVOTS`| 15                | 每节点的分界键数。              |
| `MAPLE_MIN`        | 0                 | 最小索引值。                    |
...
游标 API 使用轻量级的 `struct ma_state`（通过 `MA_STATE()` 宏在栈上分配）来
追踪树内的位置。它避免了执行顺序操作时（如扫描、批量插入相邻键或混合读-改-删
工作流）重复的根到叶遍历。
...
返回时，`mas->node`、`mas->offset`、`mas->min`、`mas->max` 和 `mas->depth`
都会更新。
...
游标必须已经定位（例如通过 `mas_walk()`）。

- **返回值：** 下一个非 NULL 条目；如果到 `max` 都没有则返回 `NULL`。

#### `void *mas_prev(struct ma_state *mas, uint64_t min)`

移动到当前位置之前的上一个非 NULL 条目。

- **返回值：** 上一个非 NULL 条目；如果到 `min` 都没有则返回 `NULL`。
